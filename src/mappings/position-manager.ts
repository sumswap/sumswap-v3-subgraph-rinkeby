import {
  ...
  SummaSwapV3Manager,
  Transfer
} from '../types/Factory/SummaSwapV3Manager'
...
import { loadPosition, loadToken } from '../utils/loaders'

function getPosition(event: ethereum.Event, tokenId: BigInt): Position | null {
  let position = loadPosition(tokenId.toString())
  if (position === null) {
    let contract = SummaSwapV3Manager.bind(event.address)
...
function updateFeeVars(position: Position, event: ethereum.Event, tokenId: BigInt): Position {
  let positionManagerContract = SummaSwapV3Manager.bind(event.address)
...
export function handleIncreaseLiquidity(event: IncreaseLiquidity): void {
  ...
  let token0 = loadToken(position.token0)
  let token1 = loadToken(position.token1)
