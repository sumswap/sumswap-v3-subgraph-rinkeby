import { Burn, Mint, Swap, Tick, Token } from '../types/schema'
...
import { findEthPerToken, getBundleEthPriceUSD, getEthPriceInUSD, getTrackedAmountUSD, sqrtPriceX96ToTokenPrices } from '../utils/pricing'
import {
  updatePoolDayData,
  updatePoolHourData,
  updateTickDayData,
  updateTokenDayData,
  updateTokenHourData,
  updateSumswapDayData
} from '../utils/intervalUpdates'
import { createTick, feeTierToTickSpacing } from '../utils/tick'

import { loadBundle, loadFactory, loadPool, loadTick, loadToken } from '../utils/loaders'
...
export function handleMint(event: MintEvent): void {
  ...
  const ethPriceUSD=getBundleEthPriceUSD(bundle);

  let amountUSD = amount0
    .times(token0.derivedETH.times(ethPriceUSD))
    .plus(amount1.times(token1.derivedETH.times(ethPriceUSD)))
  ...
  token0.totalValueLockedUSD = token0.totalValueLocked.times(token0.derivedETH.times(ethPriceUSD))

  // update token1 data
  token1.txCount = token1.txCount.plus(ONE_BI)
  token1.totalValueLocked = token1.totalValueLocked.plus(amount1)
  token1.totalValueLockedUSD = token1.totalValueLocked.times(token1.derivedETH.times(ethPriceUSD))
  ...
  pool.totalValueLockedUSD = pool.totalValueLockedETH.times(ethPriceUSD)

  // reset aggregates with new amounts
  factory.totalValueLockedETH = factory.totalValueLockedETH.plus(pool.totalValueLockedETH)
  factory.totalValueLockedUSD = factory.totalValueLockedETH.times(ethPriceUSD)
  ...
  let lowerTick = loadTick(lowerTickId)
  let upperTick = loadTick(upperTickId)

  if (lowerTick === null) {
    lowerTick = createTick(lowerTickId, lowerTickIdx, pool.id, event.block.timestamp, event.block.number)
  }

  if (upperTick === null) {
    upperTick = createTick(upperTickId, upperTickIdx, pool.id, event.block.timestamp, event.block.number)
  }
...
export function handleSwap(event: SwapEvent): void {
    ...
    let ethPriceUSD=getBundleEthPriceUSD(bundle);

    let amount0ETH = amount0Abs.times(token0.derivedETH)
    let amount1ETH = amount1Abs.times(token1.derivedETH)
    let amount0USD = amount0ETH.times(ethPriceUSD)
    let amount1USD = amount1ETH.times(ethPriceUSD)

    // get amount that should be tracked only - div 2 because cant count both input and output as volume
    let amountTotalUSDTracked = getTrackedAmountUSD(amount0Abs, token0 as Token, amount1Abs, token1 as Token).div(BigDecimal.fromString('2'))
    let amountTotalETHTracked = safeDiv(amountTotalUSDTracked, ethPriceUSD)
    ...
    bundle.save()
    token0.derivedETH = findEthPerToken(token0 as Token,ethPriceUSD)
    token1.derivedETH = findEthPerToken(token1 as Token,ethPriceUSD)
    ...
    // interval data
    let sumswapDayData = updateSumswapDayData(event)
    ...
    sumswapDayData.volumeETH = sumswapDayData.volumeETH.plus(amountTotalETHTracked)
    sumswapDayData.volumeUSD = sumswapDayData.volumeUSD.plus(amountTotalUSDTracked)
    sumswapDayData.feesUSD = sumswapDayData.feesUSD.plus(feesUSD)
