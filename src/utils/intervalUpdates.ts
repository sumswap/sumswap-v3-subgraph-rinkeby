...
import { loadFactory, loadSumswapDayData, loadPool, loadPoolDayData, loadPoolHourData, loadBundle, loadTokenDayData, loadTokenHourData, loadTickDayData } from './loaders'
import { getBundleEthPriceUSD } from './pricing'

/**
 * Tracks global aggregate data over daily windows
 * @param event
 */
export function updateSumswapDayData(event: ethereum.Event): SumswapDayData {
  let sumswap = loadFactory(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400 // rounded
  let dayStartTimestamp = dayID * 86400
  let sumswapDayData = loadSumswapDayData(dayID.toString())
  if (sumswapDayData === null) {
    sumswapDayData = new SumswapDayData(dayID.toString())
    sumswapDayData.date = dayStartTimestamp
    sumswapDayData.volumeETH = ZERO_BD
    sumswapDayData.volumeUSD = ZERO_BD
    sumswapDayData.volumeUSDUntracked = ZERO_BD
    sumswapDayData.feesUSD = ZERO_BD
  }
  sumswapDayData.tvlUSD = sumswap.totalValueLockedUSD
  sumswapDayData.txCount = sumswap.txCount
  sumswapDayData.save()
  return sumswapDayData as SumswapDayData
}
...
export function updatePoolDayData(event: ethereum.Event): PoolDayData {
  ...
  let poolDayData = loadPoolDayData(dayPoolID)
  if (poolDayData === null) {
...
export function updatePoolHourData(event: ethereum.Event): PoolHourData {
  ...
  let pool = loadPool(event.address.toHexString())
  let poolHourData = loadPoolHourData(hourPoolID)
...
export function updateTokenDayData(token: Token, event: ethereum.Event): TokenDayData {
  ...
  let tokenDayID = token.id
    ...
    .concat('-')
    .concat(dayID.toString())
  let tokenPrice = token.derivedETH.times(getBundleEthPriceUSD(bundle))

  let tokenDayData = loadTokenDayData(tokenDayID)
  ...
  tokenDayData.close = tokenPrice
  tokenDayData.priceUSD = token.derivedETH.times(getBundleEthPriceUSD(bundle))
...
export function updateTokenHourData(token: Token, event: ethereum.Event): TokenHourData {
  ...
  let tokenHourID = token.id
    ...
    .concat('-')
    .concat(hourIndex.toString())
  let tokenHourData = loadTokenHourData(tokenHourID)
  let tokenPrice = token.derivedETH.times(getBundleEthPriceUSD(bundle))
...
export function updateTickDayData(tick: Tick, event: ethereum.Event): TickDayData {
  ...
  let tickDayData = loadTickDayData(tickDayDataID)
  if (tickDayData === null) {
